...
void alloc_hook(void *dummy, ...) //struct veo_proc_handle *proc, uint64_t addr, size_t size)
{
	...
	printf("VH veo_alloc_mem hook: proc=%p addr=%lx size=%ld\n", proc, addr, size);
...
main()
{
	...
	int ret = veo_alloc_mem(proc, &vebuf, set_size);
	if (ret != 0) {
	...
	// unregister hooks
	veo_unregister_hook(&veo_alloc_mem);
	veo_unregister_hook(&veo_free_mem);

	// check unregister hooks
        void (*test_alloc_hook)(void *, ...) = veo_get_hook((void *)&veo_alloc_mem);
        if (test_alloc_hook != NULL) {
        	printf("Alloc hook function pointer retrieved: %p\n", test_alloc_hook);
        ...
        if (test_free_hook != NULL) {
        	printf("Free hook function pointer retrieved: %p\n", test_free_hook);
                printf("The hook value is unexpected, should be: NULL\n");
		exit(1);
	}
